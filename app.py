...
st.markdown("""
    <style>
    .main-title { font-size: 36px; font-weight: bold; color: #3B82F6; margin-bottom: 2px; }
    .sub-title { font-size: 16px; color: #94A3B8; margin-bottom: 25px; }
    .kpi-card {
    ...
    .kpi-value { font-size: 24px; font-weight: bold; color: #F8FAFC; }
    </style>
...
if uploaded_file is not None:
    ...
    # 4. KPI Performance Highlight Cards
    ...
    total_sales_val = f"${df[sales_col].fillna(0).sum():,.2f}" if sales_col else "N/A"
    total_profit_val = f"${df[profit_col].fillna(0).sum():,.2f}" if profit_col else "N/A"
    ...
    with col2:
        ...
        x_axis = st.selectbox("Choose Category / X-Axis:", columns, index=0)
        ...
        try:
            ...
            chart_df = chart_df.sort_values(by=y_axis, ascending=False).head(10)
        ...
        if not api_key:
            ...
            with st.spinner("Processing raw transaction matrices using Google Gemini..."):
                try:
                    ...
                    st.success("### AI CFO Consulting Output (Powered by Gemini)")
